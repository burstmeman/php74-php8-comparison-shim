#ifdef HAVE_CONFIG_H
#include "config.h"
#endif

#include "php80_string_number_comparison.h"

#include "php_ini.h"
#include "ext/standard/info.h"
#include "zend_execute.h"
#include "zend_exceptions.h"
#include "zend_operators.h"

#define PHP80_SNC_MODE_OFF 0
#define PHP80_SNC_MODE_REPORT 1
#define PHP80_SNC_MODE_ERROR 2

typedef struct {
	zend_uchar opcode;
	user_opcode_handler_t previous;
} php80_snc_opcode_handler_entry;

static int php80_snc_opcode_handler(zend_execute_data *execute_data);

static php80_snc_opcode_handler_entry php80_snc_opcode_handlers[] = {
	{ ZEND_IS_EQUAL, NULL },
	{ ZEND_IS_NOT_EQUAL, NULL },
	{ ZEND_IS_SMALLER, NULL },
	{ ZEND_IS_SMALLER_OR_EQUAL, NULL },
	{ ZEND_SPACESHIP, NULL },
	{ ZEND_CASE, NULL }
};

ZEND_DECLARE_MODULE_GLOBALS(php80_string_number_comparison)

static void php80_snc_init_globals(zend_php80_string_number_comparison_globals *globals)
{
	globals->mode = PHP80_SNC_MODE_OFF;
}

static int php80_snc_handlers_active = 0;

/* Install opcode handlers once when mode is enabled. */
static void php80_snc_enable_handlers(void)
{
	size_t index;

	if (php80_snc_handlers_active) {
		return;
	}

	for (index = 0; index < sizeof(php80_snc_opcode_handlers)
		/ sizeof(php80_snc_opcode_handlers[0]); index++) {
		php80_snc_opcode_handlers[index].previous =
			zend_get_user_opcode_handler(php80_snc_opcode_handlers[index].opcode);
		zend_set_user_opcode_handler(
			php80_snc_opcode_handlers[index].opcode,
			php80_snc_opcode_handler);
	}

	php80_snc_handlers_active = 1;
}

/* Restore previous opcode handlers when mode is disabled. */
static void php80_snc_disable_handlers(void)
{
	size_t index;

	if (!php80_snc_handlers_active) {
		return;
	}

	for (index = 0; index < sizeof(php80_snc_opcode_handlers)
		/ sizeof(php80_snc_opcode_handlers[0]); index++) {
		zend_set_user_opcode_handler(
			php80_snc_opcode_handlers[index].opcode,
			php80_snc_opcode_handlers[index].previous);
	}

	php80_snc_handlers_active = 0;
}

/* Parse INI value (zend_string) into module mode. */
static void php80_snc_set_mode_from_string(const zend_string *value)
{
	if (value == NULL || ZSTR_LEN(value) == 0) {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_OFF;
		return;
	}

	if (zend_string_equals_literal_ci(value, "off")
		|| zend_string_equals_literal_ci(value, "0")) {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_OFF;
		return;
	}

	if (zend_string_equals_literal_ci(value, "report")
		|| zend_string_equals_literal_ci(value, "1")) {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_REPORT;
		return;
	}

	if (zend_string_equals_literal_ci(value, "error")
		|| zend_string_equals_literal_ci(value, "2")) {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_ERROR;
		return;
	}

	PHP80_SNC_G(mode) = PHP80_SNC_MODE_OFF;
}

/* Parse INI value (C string) into module mode. */
static void php80_snc_set_mode_from_cstr(const char *value)
{
	if (value == NULL || value[0] == '\0') {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_OFF;
		return;
	}

	if (strcasecmp(value, "off") == 0 || strcmp(value, "0") == 0) {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_OFF;
		return;
	}

	if (strcasecmp(value, "report") == 0 || strcmp(value, "1") == 0) {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_REPORT;
		return;
	}

	if (strcasecmp(value, "error") == 0 || strcmp(value, "2") == 0) {
		PHP80_SNC_G(mode) = PHP80_SNC_MODE_ERROR;
		return;
	}

	PHP80_SNC_G(mode) = PHP80_SNC_MODE_OFF;
}

/* Apply current mode by enabling or disabling opcode handlers. */
static void php80_snc_apply_mode(void)
{
	if (PHP80_SNC_G(mode) == PHP80_SNC_MODE_OFF) {
		php80_snc_disable_handlers();
		return;
	}

	php80_snc_enable_handlers();
}

static const char *php80_snc_mode_to_string(zend_long mode)
{
	switch (mode) {
		case PHP80_SNC_MODE_REPORT:
			return "report";
		case PHP80_SNC_MODE_ERROR:
			return "error";
		case PHP80_SNC_MODE_OFF:
		default:
			return "off";
	}
}

static const char *php80_snc_opcode_to_operator(zend_uchar opcode)
{
	switch (opcode) {
		case ZEND_IS_EQUAL:
			return "==";
		case ZEND_IS_NOT_EQUAL:
			return "!=";
		case ZEND_IS_SMALLER:
			return "<";
		case ZEND_IS_SMALLER_OR_EQUAL:
			return "<=";
		case ZEND_SPACESHIP:
			return "<=>";
		case ZEND_CASE:
			return "case";
		default:
			return "comparison";
	}
}

static inline int php80_snc_is_number(const zval *value)
{
	return Z_TYPE_P(value) == IS_LONG || Z_TYPE_P(value) == IS_DOUBLE;
}

static inline int php80_snc_is_strict_numeric_string(const zend_string *value)
{
	zend_long lval;
	double dval;

	return is_numeric_string(ZSTR_VAL(value), ZSTR_LEN(value), &lval, &dval, 0) != 0;
}

static inline int php80_snc_is_non_numeric_string(const zval *value)
{
	if (Z_TYPE_P(value) != IS_STRING) {
		return 0;
	}

	return php80_snc_is_strict_numeric_string(Z_STR_P(value)) == 0;
}

static inline int php80_snc_should_report(const zval *op1, const zval *op2)
{
	/* Fast-path: report only when number vs non-numeric string. */
	return (php80_snc_is_number(op1) && php80_snc_is_non_numeric_string(op2))
		|| (php80_snc_is_number(op2) && php80_snc_is_non_numeric_string(op1));
}

static ZEND_INI_MH(php80_snc_update_mode)
{
	if (stage == PHP_INI_STAGE_RUNTIME || stage == PHP_INI_STAGE_HTACCESS) {
		return FAILURE;
	}

	php80_snc_set_mode_from_string(new_value);
	return SUCCESS;
}

PHP_INI_BEGIN()
	PHP_INI_ENTRY("php80.string_number_comparison", "Off", PHP_INI_SYSTEM, php80_snc_update_mode)
PHP_INI_END()

static int php80_snc_opcode_handler(zend_execute_data *execute_data)
{
	const zend_op *opline = execute_data->opline;
	zend_free_op free_op1 = NULL;
	zend_free_op free_op2 = NULL;
	zval *op1;
	zval *op2;

	if (PHP80_SNC_G(mode) == PHP80_SNC_MODE_OFF) {
		return ZEND_USER_OPCODE_DISPATCH;
	}

	op1 = zend_get_zval_ptr(opline, opline->op1_type, &opline->op1,
		execute_data, &free_op1, BP_VAR_R);
	op2 = zend_get_zval_ptr(opline, opline->op2_type, &opline->op2,
		execute_data, &free_op2, BP_VAR_R);

	if (op1 != NULL && op2 != NULL) {
		ZVAL_DEREF(op1);
		ZVAL_DEREF(op2);

		if (php80_snc_should_report(op1, op2)) {
			const char *op = php80_snc_opcode_to_operator(opline->opcode);
			zend_string *op1_str = zval_get_string(op1);
			zend_string *op2_str = zval_get_string(op2);

			if (PHP80_SNC_G(mode) == PHP80_SNC_MODE_ERROR) {
				zend_throw_error(NULL,
					"php80.string_number_comparison: Non-strict comparison between "
					"\"%s\" and \"%s\" using %s",
					ZSTR_VAL(op1_str),
					ZSTR_VAL(op2_str),
					op);

				zend_string_release(op1_str);
				zend_string_release(op2_str);

				if (free_op1) {
					zval_ptr_dtor_nogc(free_op1);
				}
				if (free_op2) {
					zval_ptr_dtor_nogc(free_op2);
				}

				return ZEND_USER_OPCODE_CONTINUE;
			}

			zend_error(E_DEPRECATED,
				"php80.string_number_comparison: Non-strict comparison between "
				"\"%s\" and \"%s\" using %s",
				ZSTR_VAL(op1_str),
				ZSTR_VAL(op2_str),
				op);

			zend_string_release(op1_str);
			zend_string_release(op2_str);
		}
	}

	if (free_op1) {
		zval_ptr_dtor_nogc(free_op1);
	}
	if (free_op2) {
		zval_ptr_dtor_nogc(free_op2);
	}

	return ZEND_USER_OPCODE_DISPATCH;
}

static PHP_MINIT_FUNCTION(php80_string_number_comparison)
{
	REGISTER_INI_ENTRIES();
	php80_snc_set_mode_from_cstr(INI_STR("php80.string_number_comparison"));
	php80_snc_apply_mode();

	return SUCCESS;
}

static PHP_MSHUTDOWN_FUNCTION(php80_string_number_comparison)
{
	UNREGISTER_INI_ENTRIES();
	php80_snc_disable_handlers();

	return SUCCESS;
}

static PHP_MINFO_FUNCTION(php80_string_number_comparison)
{
	php_info_print_table_start();
	php_info_print_table_header(2, "php80_string_number_comparison support", "enabled");
	php_info_print_table_row(2, "Mode", php80_snc_mode_to_string(PHP80_SNC_G(mode)));
	php_info_print_table_end();

	DISPLAY_INI_ENTRIES();
}

zend_module_entry php80_string_number_comparison_module_entry = {
	STANDARD_MODULE_HEADER,
	"php80_string_number_comparison",
	NULL,
	PHP_MINIT(php80_string_number_comparison),
	PHP_MSHUTDOWN(php80_string_number_comparison),
	NULL,
	NULL,
	PHP_MINFO(php80_string_number_comparison),
	PHP_PHP80_STRING_NUMBER_COMPARISON_VERSION,
	PHP_MODULE_GLOBALS(php80_string_number_comparison),
	php80_snc_init_globals,
	NULL,
	NULL,
	STANDARD_MODULE_PROPERTIES_EX
};

#ifdef COMPILE_DL_PHP80_STRING_NUMBER_COMPARISON
#ifdef ZTS
ZEND_TSRMLS_CACHE_DEFINE()
#endif
ZEND_GET_MODULE(php80_string_number_comparison)
#endif
